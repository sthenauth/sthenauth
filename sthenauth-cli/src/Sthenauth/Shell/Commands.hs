...
instance IsCommand alt => IsCommand (Commands alt) where
  parseCommand = hsubparser (mconcat
    ...
    ]) <|> (AdditionalCommand <$> parseCommand) <|> internalCommands
    where
...
-- | Main entry point.
main
  :: forall alt. IsCommand alt
  => Maybe (AdditionalCommand alt)
  -> Maybe DB.Runtime
  -> Maybe KeyManager
  -> Maybe Metrics.Store
  -> IO ()
main alt dbruntime keymgr mstore = do
  ...
  env <- initSthenauth config dbruntime keymgr mstore >>= checkOrDie
...
enableImplicitOptions input =
  case Options.optionsCommand input of
    InitCommand -> input { Options.optionsInit = True
